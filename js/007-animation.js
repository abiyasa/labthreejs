...
$(function () {
    ...
    // load models and its animation from model list
    ...
    // create mesh and its animation, insert into model container
    function buildModel(theModelData) {
        console.log('creating model with posY=%d', theModelData.posY);

        var modelMesh = new THREE.MorphAnimMesh(theModelData.geometry, theModelData.material);
    ...
    function updateModel() {
        ...
        var i, morph;
        var morphsNum = morphs.length;
        for (i = 0; i < morphsNum; i++) {
            morph = morphs[i];
